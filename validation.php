  ...
  <center>
    ...
    <table class="table" style="width:80%; border: 1px black solid">
      ...
      <tbody>
        ...
        $stmt = $conn->prepare("SELECT date_creation FROM mission WHERE valider = 0");
        $stmt->execute();
        foreach ($stmt->fetchAll() as $a => $b) {
          $date_process = strtotime($b['date_creation']);
          $date_process = getDate($date_process);
          $date_array = $date_process['mon'] + 1;
          if ($date_array < 10) {
            $date_array = "0" . $date_array;
          }
          if ($date_process['mday'] < 10) {
            $date_process['mday'] = "0" . $date_process['mday'];
          }
          $date_creation = $date_process['year'] . "-" . $date_array . "-" . $date_process['mday'];
        }

        if(!isset($date_creation) || !isset($date_now)){

        }else{

        $stmt = $conn->prepare("SELECT *,DATE_FORMAT(date_depart, '%W %d %M %Y') as depart, DATE_FORMAT(date_rentrer, '%W %d %M %Y') as rentrer FROM mission WHERE valider = 0 AND '" . $date_now . "' <= '" . $date_creation . "'");
        $stmt->execute();
